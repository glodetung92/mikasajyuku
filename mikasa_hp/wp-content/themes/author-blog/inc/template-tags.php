 ...
 * @package Author Blog
 */
if ( ! function_exists( 'author_blog_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function author_blog_posted_on() {
...
if ( ! function_exists( 'author_blog_time' ) ) {
	function author_blog_time() {
		$time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
		if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
			$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
		}
...
if ( ! function_exists( 'author_blog_posted_by' ) ) :
	function author_blog_posted_by( $author_image = true ) {
		...
		if ( false === $author_image ) {
			$get_author_image = __( 'Posted by', 'author-blog' );
		}
		$postedBy = sprintf(
			$posted_by_format,
...
if ( ! function_exists( 'author_blog_comment_popuplink' ) ) {
	function author_blog_comment_popuplink() {
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link(
				__( 'Post a Comment', 'author-blog' ),
				__( '1 Comment', 'author-blog' ),
				__( '% Comments', 'author-blog' ),
				$css_class,
				__( 'Comments are Closed', 'author-blog' )
			);
			echo '</span>';
		}
	}
}
function author_blog_categories() {
...
if ( ! function_exists( 'author_blog_post_tag' ) ) {
	function author_blog_post_tag() {
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'author-blog' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'author-blog' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
		return;
	}
}
if ( ! function_exists( 'author_blog_post_thumbnail' ) ) :
	/**
	 ...
	 */
	function author_blog_post_thumbnail() {
		$get_blog_layout = get_theme_mod( 'blog_layout', 'grid' );
		$thumbnail_size  = 'author-blog-grid-thumbnail';
		if ( is_single() || is_page() ) {
			$thumbnail_size = 'author-blog-thumbnail-large';
		} else {
...
function author_blog_social_activity() {
...
function author_blog_navigation() {
